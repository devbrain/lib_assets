...
namespace neutrino::assets {
	class ASSETS_EXPORT world_fs_resolver : public resource_from_stream {
		public:
			world_fs_resolver(const std::filesystem::path& main_tmx_file, const data_loader<sdl::surface, image_resource>& img_loader);

			[[nodiscard]] std::istream& get_stream() const override;
			[[nodiscard]] const data_loader<sdl::surface, image_resource>& get_image_loader () const;
			[[nodiscard]] std::string resolve(const std::string& path) const;
		private:
			std::filesystem::path m_base_path;
			const data_loader<sdl::surface, image_resource>& m_img_loader;
