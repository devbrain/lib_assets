...
#include "world/tmx/test/data/wang/isometric_staggered_grass_and_water.h"
#include "world/tmx/test/data/wang/js_isometric_staggered_grass_and_water.h"

using namespace neutrino::assets::tmx;

static void test_wang(const map& the_map) {
	REQUIRE(the_map.orientation () == neutrino::assets::orientation_t::STAGGERED);
	REQUIRE(the_map.render_order () == neutrino::assets::render_order_t::RIGHT_DOWN);
	REQUIRE(the_map.infinite ());
	REQUIRE(the_map.stagger_axis () == neutrino::assets::stagger_axis_t::Y);
	REQUIRE(the_map.stagger_index () == neutrino::assets::stagger_index_t::ODD);
...
TEST_CASE("Test Wang") {
	SUBCASE("wang json test") {
		auto the_map = test::load_map(js_isometric_staggered_grass_and_water,
		                              js_isometric_staggered_grass_and_water_length);
		test_wang(the_map);
	}
