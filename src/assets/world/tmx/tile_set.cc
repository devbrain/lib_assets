...
namespace neutrino::assets::tmx {
	...
	sdl::rect tile_set::get_coords(unsigned id, const sdl::area_type& size) const noexcept {
		unsigned width = (size.w - 2 * m_margin + m_spacing) / (m_tilewidth + m_spacing); // number of tiles
		unsigned height = (size.h - 2 * m_margin + m_spacing) / (m_tileheight + m_spacing); // number of tiles
