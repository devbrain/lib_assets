...
namespace neutrino::assets {
  ...
  void texture_atlas::add (sdl::texture&& texture, sprite_coords_t&& coords) {
    m_atlas.emplace_back (std::move(texture), std::move(coords));
  }

  void texture_atlas::add (sdl::texture&& texture, int width, int height) {
  ...
  std::tuple<const sdl::texture&, sdl::rect> texture_atlas::get(texture_id_t id, tile_id_t tile_id) const {
