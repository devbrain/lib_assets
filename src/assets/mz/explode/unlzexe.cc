...
static void
build_rellocs_90 (neutrino::assets::mz::io::input& file, std::vector<neutrino::assets::mz::rellocation>& rellocs) {
	neutrino::assets::mz::byte_reader f (file);
	int16_t seg = 0;
	do {
		uint16_t t = f.word ();
		int c = t & 0xFFFF;
		...
		for (; c > 0; c--) {
			...
			rellocs.push_back (neutrino::assets::mz::rellocation (static_cast <uint16_t> (seg), offs));
		}
...
// ----------------------------------------------------------------
static void
build_rellocs_91 (neutrino::assets::mz::io::input& file, std::vector<neutrino::assets::mz::rellocation>& rellocs) {
	int16_t seg = 0;
	int16_t offs = 0;
	int16_t span = 0;
	neutrino::assets::mz::byte_reader f (file);
	while (true) {
		...
		offs &= 0x0f;
		rellocs.push_back (
			neutrino::assets::mz::rellocation (static_cast <uint16_t> (seg), static_cast <uint16_t>(offs)));
	};
}

static uint32_t
unpak_code (neutrino::assets::mz::output_exe_file& oexe, neutrino::assets::mz::io::input& input, uint32_t offset) {
	input.seek (offset);
	neutrino::assets::mz::bit_reader bitstream (input);
