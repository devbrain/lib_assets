...
namespace assets::pefile
{
	...
	public:
		...
		bool ReadName(const uint32_t nResource, wchar_t *Str, uint32_t Len) const;
		bool GetResourceInfo(uint32_t nResource, wchar_t *Str, uint32_t Len, uint32_t *Offset, int *TypeIndex) const;

	private:
		static bool DecodeInt(const uint8_t*  pAddress, int *Value, unsigned int *uSize);
	};
