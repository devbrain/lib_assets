...
namespace assets::pefile {
	namespace detail {
		...
		template <typename H>
		struct tuple_type_cnv<bsw::mp::type_list<H>> {
		...
		struct tuple_type_cnv<bsw::mp::type_list<Head, Tail...>> {
			using A = typename tuple_type_cnv<typename Head::type>::type;
			using type = typename append_to_type_seq<A,
													 typename tuple_type_cnv<bsw::mp::type_list<Tail...>>::type>::type;
		};
		...
		struct elem_reader {
			using input_type = typename bsw::mp::type_list_at_t<INDEX, ROW>;
		...
		struct inst_reader_impl {
			static void read (INSTANCE& instance, uint8_t StreamOffsetSize, bsw::istream_wrapper_c& is) {
				...
				inst_reader_impl<ROW,
								 INSTANCE,
								 INDEX + 1,
								 INDEX + 1 < ROW::size ()>::read (instance, StreamOffsetSize, is);
			}
		};
		...
		template <typename T>
		struct memory_size_impl<bsw::mp::type_list<T>> {
			static size_t eval (uint8_t StreamOffsetSize) {
		...
		struct memory_size_impl<bsw::mp::type_list<H, Ts...>> {
			static size_t eval (uint8_t StreamOffsetSize) {
				return memory_size_impl<typename H::type>::eval (StreamOffsetSize) +
					   memory_size_impl<bsw::mp::type_list<Ts...>>::eval (StreamOffsetSize);
			}
		};
	...
	struct select_type_by_id<ECMA_ID, bsw::mp::type_list<Head>> {
	...
	struct select_type_by_id<ECMA_ID, bsw::mp::type_list<Head, Tail...>> {
		using value = typename std::conditional<ECMA_ID == Head::ECMA_ID,
												Head,
												typename select_type_by_id<ECMA_ID,
																		   bsw::mp::type_list<Tail...>>::value>::type;
	};
	...
	template <>
	struct generic_memory_size<NullType> {
		static std::size_t eval (uint8_t StreamOffsetSize) {
			return 0;
		}
	};
...
#define CLR_TABLES_SYSTEM_BEGIN(PFX)                \
    struct PFX                                        \
    {                                                \
        typedef bsw::mp::type_list<>

#define CLR_TABLE_DEF_BEGIN(NAME, IDVAL)            \
    PPCAT(tables_pre_, NAME);                        \
    struct NAME                                        \
    {                                                \
...
#define CLR_ROW(TYPE, NAME)                                    \
    ...
    {                                                        \
        ...
        {                                                    \
            return PPCAT (rows_pre_, NAME)::size ();    \
        }                                                    \
    };                                                        \
    typedef bsw::mp::type_list_append_t <NAME, PPCAT(rows_pre_, NAME)>

#define CLR_TABLE_DEF_END(NAME)                                    \
    TABLE_ROWS;                                                    \
    typedef detail::tuple_type_cnv<TABLE_ROWS>::type instance;    \
    };                                                            \
    typedef bsw::mp::type_list_append_t <NAME, PPCAT(tables_pre_, NAME)>
