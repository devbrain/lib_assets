...
namespace assets::pefile {
	...
	// -------------------------------------------------------------------
	static const uint16_t IMAGE_OPTIONAL_HEADER_PE32 = 0x10B;
	static const uint16_t IMAGE_OPTIONAL_HEADER_PE32_PLUS = 0x20B;
...
;\
	...
	void file_c::_check_coff_header (abstract_reporter_c& reporter) {
		if (!check_in_enum<COFF_HEADER::IMAGE_FILE_MACHINE> (m_coff_header->Machine)) {
			reporter.invalid_enum_value (abstract_reporter_c::COFF_HEADER_MACHINE_TYPE, m_coff_header->Machine);
		}
		if (!check_in_flags<COFF_HEADER::IMAGE_FILE_CHARACTERISTICS> (m_coff_header->Characteristics)) {
			reporter
	...
	// --------------------------------------------------------------------------------
	void file_c::_check_optional_header (abstract_reporter_c& reporter) const {
		...
		if (!check_in_flags<OPTIONAL_HEADER::IMAGE_SUBSYSTEM> (m_optional_header.Subsystem)) {
			reporter.unknown_flag_set (abstract_reporter_c::OPTIONAL_HEADER_SUBSYSTEM, m_optional_header.Subsystem);
		}
		if (!check_in_flags<OPTIONAL_HEADER::IMAGE_DLLCHARACTERISTICS> (m_optional_header.DllCharacteristics)) {
