...
namespace assets::pefile {
	...
	void exe_file_c::_load () {
		...
		if (coff.Machine != COFF_HEADER::IMAGE_FILE_MACHINE_I386
			&& coff.Machine != COFF_HEADER::IMAGE_FILE_MACHINE_AMD64) {
			std::ostringstream os;
			os << "Unsupported architecture " << static_cast <COFF_HEADER::IMAGE_FILE_MACHINE>(coff.Machine);
			throw std::logic_error (os.str ());
		}
		if ((coff.Characteristics & COFF_HEADER::IMAGE_FILE_EXECUTABLE_IMAGE)
