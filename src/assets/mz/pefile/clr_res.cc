...
namespace assets::pefile {
	clr_resource_c::clr_resource_c () = default;
...
#define RESOURCES_MAGIC_NUMBER            0xBEEFCACE
	...
	bool clr_resource_c::ReadName (const uint32_t nResource, wchar_t* Str, uint32_t Len) const {
		...
		const auto* ptr = (const uint8_t*)(NameOffset + ((ULONG_PTR)pNames));
	...
	bool
	clr_resource_c::GetResourceInfo (uint32_t nResource, wchar_t* Str, uint32_t Len, uint32_t* Offset, int* TypeIndex) const {
		//
		// Read name
		//
		...
		const auto* pData = (const uint8_t*)(DataOffset + ((ULONG_PTR)pDataSection));
