...
namespace assets::detail {
	...
	int color_depth (const bits_per_pixel_t& bpp) {
		switch (bpp) {
			...
			case eBPP4:return 4;
			case eBPP8:return 8;
			case eBPP16:return 16;
			case eBPP24:return 24;
			case eBPP32:return 32;
		}
		return 0;
	}
	...
	struct bmpfile_header_s {
		...
		explicit bmpfile_header_s (bsw::istream_wrapper_c& isd) {
			isd >> magic[0] >> magic[1]
	...
	struct core_win3_info_header_s {
		...
		core_win3_info_header_s (bsw::istream_wrapper_c& icd, uint32_t size)
			: biSize (size) {
			icd >> biWidth
				>> biHeight
				>> biPlanes
		...
		void operator() (bmp_info_s& bi) const {
			bi.width = biWidth;
			if (biHeight < 0) {
				bi.height = -biHeight;
				bi.top_down = false;
			...
			switch (biBitCount) {
				...
				case 8:bi.bpp = eBPP8;
					break;
				case 16:bi.bpp = eBPP16;
					break;
				case 24:bi.bpp = eBPP24;
					break;
				case 32:bi.bpp = eBPP32;
					break;
				default: {
					RAISE_EX("Unsupported color depth ", biBitCount);
				}
			}
	...
	struct os2_v1_info_header_s {
		...
		os2_v1_info_header_s (bsw::istream_wrapper_c& icd, uint32_t size)
			: biSize (size) {
			icd >> biWidth
				>> biHeight
				>> biPlanes
	...
	struct os2_v2_info_header_s {
		...
		os2_v2_info_header_s (bsw::istream_wrapper_c& icd, uint32_t size)
			: Size (size) {
			icd >> Width;
			icd >> Height;
			icd >> NumPlanes;
		...
		void operator() (bmp_info_s& bi) const {
			...
			switch (BitsPerPixel) {
				...
				case 24:bi.bpp = eBPP24;
					...
					break;
				default: {
					RAISE_EX("Unsupported color depth ", BitsPerPixel);
				}
			}
			switch (Compression) {
				case 0:bi.compression_method = eBMP_RGB;
					break;
				case 1:bi.compression_method = eBMP_RLE8;
					break;
				case 2:bi.compression_method = eBMP_RLE4;
					break;
				default: {
					RAISE_EX("Unsupported compression method ", Compression);
				}
			}
			bi.rgb_length = 4;
		}
	...
	struct core_info_header_s {
		...
		core_info_header_s (bsw::istream_wrapper_c& icd, uint32_t size)
		...
		void operator() (bmp_info_s& bi) const {
			...
			switch (biCompression) {
				...
				case 2:bi.compression_method = eBMP_RLE4;
					break;
				case 3:bi.compression_method = eBMP_BITFIELDS;
					break;
				case 4:bi.compression_method = eBMP_JPEG;
					break;
				case 5:bi.compression_method = eBMP_PNG;
					break;
				default: {
					RAISE_EX("Unsupported compression method ", biCompression);
				}
			}
			if (biClrUsed == 0) {
				if (biClrImportant == 0) {
					switch (biOS2Header.biBitCount) {
						case 1:bi.colors_in_palette = 2;
							break;
						case 2:bi.colors_in_palette = 4;
							break;
						case 4:bi.colors_in_palette = 16;
							break;
						case 8:bi.colors_in_palette = 256;
							break;
						default:bi.colors_in_palette = 0;
					}
				} else {
	...
	struct v2_info_header_s {
		...
		v2_info_header_s (bsw::istream_wrapper_c& icd, uint32_t size)
		...
		void operator() (bmp_info_s& bi) const {
			biCoreHeader (bi);
			bi.rmask = bmiRedMask;
			bi.gmask = bmiGreenMask;
			bi.bmask = bmiGreenMask;
		}
	};
	...
	struct v3_info_header_s {
		...
		v3_info_header_s (bsw::istream_wrapper_c& icd, uint32_t size)
	...
	static
	bsw::istream_wrapper_c& operator>> (bsw::istream_wrapper_c& isd, ciexyz_s& x) {
	...
	static
	bsw::istream_wrapper_c& operator>> (bsw::istream_wrapper_c& isd, ciexyz_tripple_s& x) {
	...
	struct v4_info_header_s {
		...
		v4_info_header_s (bsw::istream_wrapper_c& icd, uint32_t size)
	...
	struct v5_info_header_s {
		...
		v5_info_header_s (bsw::istream_wrapper_c& icd, uint32_t size)
	...
	std::ostream& operator<< (std::ostream& os,
		...
		switch (kind) {
			case eBMP_BM:os << "Windows";
				break;
			case eBMP_BA:os << "OS/2 Bitmap Array";
				break;
			case eBMP_CI:os << "OS/2 Color Icon";
				break;
			case eBMP_CP:os << "OS/2 Color Pointer";
				break;
			case eBMP_IC:os << "OS/2 Icon";
				break;
			case eBMP_PT:os << "OS/2 Pointer";
				break;
		}
		return os;
	}
	...
	std::ostream& operator<< (std::ostream& os,
		...
		switch (bmp_compression_method) {
			case eBMP_RGB:os << "RGB";
				break;
			case eBMP_RLE8:os << "RLE-8";
				break;
			case eBMP_RLE4:os << "RLE-4";
				break;
			case eBMP_BITFIELDS:os << "BITFIELDS";
				break;
			case eBMP_JPEG:os << "JPG";
				break;
			case eBMP_PNG:os << "PNG";
				break;
		}
		return os;
	}
	...
	// --------------------------------------------------------------------------
	void load_bmp_info (std::istream& inp, bmp_info_s& bi) {
		bsw::istream_wrapper_c isd (inp);
	...
	class simple_pixels_reader_c {
	 ...
	 public:
		simple_pixels_reader_c (bsw::istream_wrapper_c& isd, const bmp_info_s& bi)
			: m_isd (isd),
			  m_bi (bi),
			  m_row_size (pixels_row_size (BITS, bi.width)),
			  m_row (m_row_size, 0),
			  m_current (0),
			  m_chunk (0) {
		}
		...
		void next_pixel (pixel_t& pixel) {
			...
			if (m_current >= m_row_size) {
				RAISE_EX("next_pixel is out of bounds");
			}
	 ...
	 private:
		bsw::istream_wrapper_c& m_isd;
		const bmp_info_s& m_bi;
	 protected:
	...
	class rle_pixels_reader_c {
	 public:
		typedef uint8_t pixel_t;
	 public:
		explicit rle_pixels_reader_c (bsw::istream_wrapper_c& isd)
			: m_isd (isd),
			  m_current_x (0),
			  m_current_y (0),
			  m_state (eUNKNOWN),
...
#define DEFINE_RGB_TRAITS(BITS, WORD, RMASK, GMASK, BMASK, AMASK, RSHIFT, GSHIFT, BSHIFT, ASHIFT, SCALE)    \
	...
	class rbg_pixels_reader_c {
	 ...
	 public:
		rbg_pixels_reader_c (bsw::istream_wrapper_c& isd, const bmp_info_s& bi)
			: m_isd (isd),
			  m_bi (bi),
			  m_row_size (pixels_row_size (BITS, bi.width)),
			  m_row (bi.width * sizeof (typename rgb_traits_s<BITS>::word_t), 0),
			  m_current (0) {
		...
		void next_pixel (pixel_t& pixel) {
			...
			if (BITS != 16) {
				pixel.r = static_cast<uint8_t>(
					(((v & rgb_traits_s<BITS>::rmask) >> rgb_traits_s<BITS>::rshift) & 0xFF)
						<< rgb_traits_s<BITS>::scale);
				pixel.g = static_cast<uint8_t>(
					(((v & rgb_traits_s<BITS>::gmask) >> rgb_traits_s<BITS>::gshift) & 0xFF)
						<< rgb_traits_s<BITS>::scale);
				pixel.b = static_cast<uint8_t>(
					(((v & rgb_traits_s<BITS>::bmask) >> rgb_traits_s<BITS>::bshift) & 0xFF)
						<< rgb_traits_s<BITS>::scale);
				if (rgb_traits_s<BITS>::amask == 0) {
					pixel.a = 0xFF;
				} else {
					pixel.a = static_cast<uint8_t>(
						(((v & rgb_traits_s<BITS>::amask) >> rgb_traits_s<BITS>::ashift) & 0xFF)
							<< rgb_traits_s<BITS>::scale);
				}
			} else {
				if (m_bi.compression_method != eBMP_BITFIELDS) {
					pixel.r = static_cast<uint8_t>(
						(((v & rgb_traits_s<BITS>::rmask) >> rgb_traits_s<BITS>::rshift) & 0xFF)
							<< rgb_traits_s<BITS>::scale);
					pixel.g = static_cast<uint8_t>(
					...
					pixel.r = static_cast<uint8_t>((((v & m_bi.rmask) >> m_bi.rshift) & 0xFF) << m_bi.rscale);
					pixel.g = static_cast<uint8_t>((((v & m_bi.gmask) >> m_bi.gshift) & 0xFF) << m_bi.gscale);
					pixel.b = static_cast<uint8_t>((((v & m_bi.bmask) >> m_bi.bshift) & 0xFF) << m_bi.bscale);
					pixel.a = 0xFF;
				}
			}
			m_current += BYTES;
...
#define DEFINE_READER(BITS, NAME)                    \
	...
	template <int BITS>
	struct pixel_setter {
		using reader_t = typename simple_reader_traits_s<BITS>::reader_t;
		using pixel_t = typename reader_t::pixel_t;

		static void set(SDL_Surface* img, unsigned int x, unsigned int y, const pixel_t& px) {
			auto * const target_pixel = (Uint32 *) ((Uint8 *) img->pixels
													+ y * img->pitch
													+ x * img->format->BytesPerPixel);
			*target_pixel = SDL_MapRGB (img->format, px.r, px.g, px.b);
		}
	};

	template <>
	struct pixel_setter <8> {
		using reader_t = typename simple_reader_traits_s<8>::reader_t;
		using pixel_t = typename reader_t::pixel_t;

		static void set(SDL_Surface* img, unsigned int x, unsigned int y, const pixel_t& px) {
			const std::size_t pitch = img->pitch;
			const std::size_t total_size = img->pitch*img->h;
			char* start = static_cast<char*>(img->pixels);
			char* row = start + y*pitch;
			std::size_t offs = x*sizeof (pixel_t);
			ENFORCE(y*pitch+offs + sizeof (pixel_t) < total_size);
			*(row + offs) = (char)px;
		}
	};

	template <>
	struct pixel_setter <4> {
		using reader_t = typename simple_reader_traits_s<4>::reader_t;
		using pixel_t = typename reader_t::pixel_t;
	...
	template <>
	struct pixel_setter <2> {
		using reader_t = typename simple_reader_traits_s<2>::reader_t;
		using pixel_t = typename reader_t::pixel_t;
	...
	template <>
	struct pixel_setter <1> {
		using reader_t = typename simple_reader_traits_s<1>::reader_t;
		using pixel_t = typename reader_t::pixel_t;
	...
	template <>
	struct pixel_setter <32> {
		using reader_t = typename simple_reader_traits_s<32>::reader_t;
		using pixel_t = typename reader_t::pixel_t;
		...
		static void set(SDL_Surface* img, unsigned int x, unsigned int y, const pixel_t& px) {
			auto * const target_pixel = (Uint32 *) ((Uint8 *) img->pixels
													  + y * img->pitch
													  + x * img->format->BytesPerPixel);
			*target_pixel = SDL_MapRGBA (img->format, px.r, px.g, px.b, px.a);
		}
	};


	template <int BITS>
	void uncompressed_loader (bsw::istream_wrapper_c& isd, SDL_Surface* img, const bmp_info_s& bi) {
		...
		for (unsigned int y = 0; y < bi.height; y++) {
			...
			for (unsigned int x = 0; x < bi.width; x++) {
				...
				pixel_setter<BITS>::set (img,x, inv_y, pixel);
			}
		}
	}
	...
	// --------------------------------------------------------------------------
	template <int BITS>
	void compressed_loader (bsw::istream_wrapper_c& isd, SDL_Surface* img, const bmp_info_s& bi) {
	...
	// =================================================================================
	static uint32_t count_zero_bits (uint32_t v) {
		uint32_t mask = 1;
		uint32_t r = 0;
		for (int i = 0; i < 32; i++) {
			if (v & mask) {
	...
	static uint32_t count_scale_bits (uint32_t v) {
		uint32_t mask = 1;
		uint32_t r = 0;
		for (int i = 0; i < 32; i++) {
			if (v & mask) {
	...
	bool is_bmp (std::istream& is) {
		...
		return std::any_of (sigs.begin(), sigs.end(), [&fh](auto s) {
		  return test_magic (fh, s);
		});
	}
	...
	image_info bmp_info(std::istream &is) {
		bsw::istream_wrapper_c isd (is);
		bmp_info_s bi;
		const file_size_t stream_start = isd.current_pos ();
		load_bmp_info (is, bi);

		file_size_t curr_pos = isd.current_pos ();
		if (bi.offset_to_data < curr_pos) {
			RAISE_EX("BMP file is corrupted");
		...
		if ((bi.colors_in_palette > 0) && (num_colors < bi.colors_in_palette)) {
			RAISE_EX("BMP file is corrupted. Number of colors mismatch."
					 "estimated number of colors ", num_colors, ", desired number of colors ",
					 bi.colors_in_palette);
		}

		neutrino::sdl::pixel_format::format bpp;

		switch (bi.bpp) {
			case eBPP1:
			case eBPP2:
			case eBPP4:
			case eBPP8:bpp = neutrino::sdl::pixel_format::format::INDEX8;
				break;

			case eBPP16:
			case eBPP24:bpp = neutrino::sdl::pixel_format::format::RGB24;
				break;
			case eBPP32:bpp = bpp = neutrino::sdl::pixel_format::format::RGBA32;
				break;
		}
		return {bi.width, bi.height, neutrino::sdl::pixel_format(bpp)};
	}

	neutrino::sdl::surface bmp_load (std::istream& inp) {
		bsw::istream_wrapper_c isd (inp);
		bmp_info_s bi;
		const file_size_t stream_start = isd.current_pos ();
		...
		switch (bi.bpp) {
			...
			case eBPP32:
				bpp = eBPP32;
				format = neutrino::sdl::pixel_format::format::RGBA32;
				break;
		}

		neutrino::sdl::surface surface(bi.width, bi.height, neutrino::sdl::pixel_format(format));


		if (bpp == eBPP16 && bi.compression_method == eBMP_BITFIELDS) {
			isd >> bi.rmask;
			isd >> bi.gmask;
			isd >> bi.bmask;

			bi.rshift = count_zero_bits (bi.rmask);
			bi.gshift = count_zero_bits (bi.gmask);
			bi.bshift = count_zero_bits (bi.bmask);

			bi.rscale = 8 - count_scale_bits (bi.rmask);
			bi.gscale = 8 - count_scale_bits (bi.gmask);
			bi.bscale = 8 - count_scale_bits (bi.bmask);

			bi.amask = 0;
		} else {
			if (bpp == eBPP8) {
				auto *colors = surface->format->palette->colors;
				surface->format->palette->ncolors = bi.colors_in_palette;
				for (unsigned int i = 0; i < bi.colors_in_palette; i++) {
					uint8_t rgba[4];
					isd.read ((char*)rgba, bi.rgb_length);
					colors[i].b = rgba[0] & 0xFF;
					colors[i].g = rgba[1] & 0xFF;
					colors[i].r = rgba[2] & 0xFF;
					colors[i].a = rgba[3] & 0xFF;;
				}
			}
		}
		...
		switch (bi.bpp) {
			case eBPP1:uncompressed_loader<1> (isd, surface.handle(), bi);
				break;
			case eBPP2:uncompressed_loader<2> (isd, surface.handle(), bi);
				break;
			case eBPP4:
				switch (bi.compression_method) {
					case eBMP_RLE4:compressed_loader<4> (isd, surface.handle(), bi);
						break;
					case eBMP_RGB:uncompressed_loader<4> (isd, surface.handle(), bi);
						break;
					default: {
						RAISE_EX("Unsupported compression method ", bi.compression_method, " for 4-bit depth BMP");
					}
				}
				break;
			case eBPP8:
				switch (bi.compression_method) {
					case eBMP_RLE8:compressed_loader<8> (isd, surface.handle(), bi);
						break;
					case eBMP_RGB:uncompressed_loader<8> (isd, surface.handle(), bi);
						break;
					default: {
						RAISE_EX("Unsupported compression method ", bi.compression_method, " for 8-bit depth BMP");
					}
				}
				break;
			case eBPP16:uncompressed_loader<16> (isd, surface.handle(), bi);
				break;
			case eBPP24:uncompressed_loader<24> (isd, surface.handle(), bi);
				break;
			case eBPP32:uncompressed_loader<32> (isd, surface.handle(), bi);
				break;
		}
		return surface;
	}
