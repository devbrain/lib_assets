...
#include "image/thirdparty/lodepng.h"
#include <bsw/strings/wchar.hh>

#define MY_ASSERT(EXP) if (!( EXP )) { throw std::runtime_error (#EXP " failed"); }
...
#define DEFINE_READER(BITS, NAME)                    \
    template <>                                      \
		...
		class abstract_picture_c {
		 public:
			abstract_picture_c (unsigned int width, unsigned int height)
				: m_width (width),
				  m_height (height) {
				m_image.resize (width * height * 4, 0xFF);
			}
			...
			void put_pixel (unsigned int x, unsigned int y, uint8_t color) {
				MY_ASSERT (color < m_palette.size ());
				put_pixel (x, y, m_palette[color]);
			}

			void put_pixel (unsigned int x, unsigned int y, const rgba_s color) {
				MY_ASSERT (x < m_width);
				MY_ASSERT (y < m_height);
				const std::size_t offs = 4 * (m_width * y + x);
				MY_ASSERT (offs < m_image.size ());

				m_image[offs + 0] = color.r;
				m_image[offs + 1] = color.g;
				m_image[offs + 2] = color.b;
				m_image[offs + 3] = 0xFF; //color.a;
			}

			void save (std::vector<unsigned char>& png) {
		 ...
		 private:
			...
			rgba_vector_t m_palette;
		};

		// =================================================================================

		template <int BITS>
		void uncompressed_loader (bsw::istream_wrapper_c& isd, abstract_picture_c* img, const bmp_info_s& bi) {
		...
		// --------------------------------------------------------------------------
		template <int BITS>
		void compressed_loader (bsw::istream_wrapper_c& isd, abstract_picture_c* img, const bmp_info_s& bi) {
		...
		// =================================================================================
		abstract_picture_c* load_bmp (std::istream& inp, bmp_info_s& bi) {
			bsw::istream_wrapper_c isd (inp);
			...
			if ((bi.colors_in_palette > 0) && (num_colors < bi.colors_in_palette)) {
				std::ostringstream os;
				os << "BMP file is corrupted. Number of colors mismatch."
			...
			switch (bi.bpp) {
				...
				case eBPP2:
				case eBPP4:
				case eBPP8:bpp = eBPP8;
					break;

				case eBPP16:bpp = eBPP16;
					break;
				case eBPP24: bpp = eBPP24;
					break;
				case eBPP32:bpp = eBPP32;
					break;
			}

			abstract_picture_c* img = new abstract_picture_c (bi.width, bi.height);
			...
			switch (bi.bpp) {
				case eBPP1:uncompressed_loader<1> (isd, img, bi);
					break;
				case eBPP2:uncompressed_loader<2> (isd, img, bi);
					break;
				case eBPP4:
					switch (bi.compression_method) {
						case eBMP_RLE4:compressed_loader<4> (isd, img, bi);
							break;
						case eBMP_RGB:uncompressed_loader<4> (isd, img, bi);
							break;
						default: {
							std::ostringstream os;
				...
				case eBPP8:
					switch (bi.compression_method) {
						case eBMP_RLE8:compressed_loader<8> (isd, img, bi);
							break;
						case eBMP_RGB:uncompressed_loader<8> (isd, img, bi);
							break;
						default: {
							std::ostringstream os;
					...
					break;
				case eBPP16:uncompressed_loader<16> (isd, img, bi);
					break;
				case eBPP24:uncompressed_loader<24> (isd, img, bi);
					break;
				case eBPP32:uncompressed_loader<32> (isd, img, bi);
					break;
			}
			return img;
		}
		...
		// =============================================================================================
		bool load_bmp (std::istream& stream, std::vector<unsigned char>& out) {
			if (!stream) {
				return false;
			}
			try {
				bmp_info_s bi;
				abstract_picture_c* img = load_bmp (stream, bi);
				img->save (out);
				delete img;
			}
			catch (...) {
		...
		// --------------------------------------------------------------------------------------------------
		bool load_bmp (const std::string& fname, std::vector<unsigned char>& out) {
			std::ifstream ifs (fname, std::ios::binary);
			return load_bmp (ifs, out);
		}

		// --------------------------------------------------------------------------------------------------
		bool load_bmp (const std::wstring& fname, std::vector<unsigned char>& out) {
			std::ifstream ifs (bsw::wstring_to_utf8 (fname), std::ios::binary);
			return load_bmp (ifs, out);
		}

		// --------------------------------------------------------------------------------------------------
		bool convert_bmp (std::istream& stream, std::vector<unsigned char>& png) {
			return load_bmp (stream, png);
		}

		// --------------------------------------------------------------------------------------------------
		bool convert_bmp (const std::string& fname, std::vector<unsigned char>& png) {

			return load_bmp (fname, png);
		}
