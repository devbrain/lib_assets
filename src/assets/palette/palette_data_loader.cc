...
static constexpr auto VGA_PALETTE_LENGTH = 256;
...
static neutrino::sdl::palette from_standard(int colors) {
    neutrino::sdl::palette pal(colors);
    for (int i=0; i<colors; i++) {
        const uint32_t p = bsw::byte_order::from_little_endian(vga_palette[i]);
        const auto r = (p & 0x00FF0000) >> 16;
        const auto g = (p & 0x0000FF00) >> 8;
        const auto b = (p & 0x000000FF);
        const neutrino::sdl::color c(r & 0xFF, g&0xFF, b&0xFF);
        pal[i] = c;
    }
    return pal;
}
...
namespace neutrino::assets {
    class binary_palette_loader : public abstract_resource_loader<sdl::palette> {
        bool accept(std::istream& is) const override {
            return true;
        }

        sdl::palette load(std::istream& is) const override {
            std::vector<sdl::color> colors;
            int c = 0;
            while (c < 256 && !is.eof()) {
                char rgb[3];
                is.read(rgb, 3);
                if (!is) {
                    RAISE_EX("Corrupted palette");
                }
                c++;
            ...
            sdl::palette p(c);
            p.fill(colors.data(), colors.data() + colors.size());
            return p;
        }
    };

    palette_data_loader::palette_data_loader() {
    ...
    const sdl::palette& load_standard_vga_palette() {
        static auto p = from_standard(VGA_PALETTE_LENGTH);
        return p;
    }
}