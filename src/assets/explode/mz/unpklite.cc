...
#include "explode/mz/unpklite.hh"
...
namespace
{
    ...
    // ===================================================================
    void adjust_length_code_2000(uint16_t& length_code, assets::mz::bit_reader& f, bool uncompressed_region)
    {
        // 4627
        while (true)
        {
    ...
    // -------------------------------------------------------------------
    void adjust_length_code_n2000(uint16_t& length_code, assets::mz::bit_reader& f, bool uncompressed_region)
    {
        // 474e
        while (true)
        {
    ...
    typedef void (* adjust_length_code_fn)(uint16_t& length_code,
                                           assets::mz::bit_reader& f,
                                           bool uncompressed_region);

    uint16_t get_base_offset(assets::mz::bit_reader& f)
    {
        // 4b05
        while (true)
        {
    ...
    void build_rellocs(uint16_t h_pklite_info,
                       assets::mz::struct_reader<uint32_t>& fr,
                       std::vector<assets::mz::rellocation>& rellocs)
    {

        assets::mz::struct_reader<uint16_t> f(fr.input());
        ...
        if ((h_pklite_info & 0x1000) == 0)
        {
            while (true)
            {
                ...
                while (has_bytes < length_code)
                {
                    ...
                    rellocs.push_back(assets::mz::rellocation(seg, rel));
                    relocs_count++;
                    has_bytes++;
                }
            }
