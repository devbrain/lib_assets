...
#include "explode/mz/unlzexe.hh"
#include "explode/mz/struct_reader.hh"
#include <bsw/exception.hh>

static void build_rellocs_90(assets::mz::io::input& file, std::vector<assets::mz::rellocation>& rellocs)
{
    assets::mz::byte_reader f(file);
    int16_t seg = 0;
    do
    {
        uint16_t t = f.word();
        ...
        for (; c > 0; c--)
        {
            ...
            rellocs.push_back(assets::mz::rellocation(static_cast <uint16_t> (seg), offs));
        }
...
// ----------------------------------------------------------------
static void build_rellocs_91(assets::mz::io::input& file, std::vector<assets::mz::rellocation>& rellocs)
{
    int16_t seg = 0;
    int16_t offs = 0;
    int16_t span = 0;
    ...
    while (true)
    {
        ...
        offs &= 0x0f;
        rellocs.push_back(
                assets::mz::rellocation(static_cast <uint16_t> (seg), static_cast <uint16_t>(offs)));
    };
}

static uint32_t unpak_code(assets::mz::output_exe_file& oexe, assets::mz::io::input& input, uint32_t offset)
{
    input.seek(offset);
    assets::mz::bit_reader bitstream(input);
