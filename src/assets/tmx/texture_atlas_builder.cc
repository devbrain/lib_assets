...
namespace neutrino::assets {
	namespace {
		sdl::rect get_coords(unsigned id, unsigned canvas_w, unsigned canvas_h, const tile_sheet_info& inf) {
	...
	detail::base_image::base_image(std::optional <sdl::color> transparent_color,
	...
	std::optional <sdl::color> detail::base_image::transparent_color() const noexcept {
		return m_transparent_color;
	}

	std::optional <sdl::area_type> detail::base_image::dimensions() const noexcept {
		return m_dimensions;
	}
	...
	named_image::named_image(const std::optional <sdl::color>& transparent_color,
	                         const std::optional <sdl::area_type>& dimensions,
	...
	image_data::image_data(const std::optional <sdl::color>& transparent_color,
	                       const std::optional <sdl::area_type>& dimensions,
	                       const std::optional <std::string>& format,
	...
	texture_atlas texture_atlas_builder::build(sdl::renderer& renderer,
	                                           const std::function <sdl::surface (const std::string&)>& loader) const {
		texture_atlas out;
			...
			std::visit(
				bsw::overload(
					...
					[&transparent_color, &surface](const image_data& data) {
						...
						bsw::io::memory_input_stream is(buffer.data(), buffer.size());
						// TODO
						//surface = hal::load (is);
						if (!surface) {
