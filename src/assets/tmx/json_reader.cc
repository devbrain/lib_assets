...
namespace neutrino::tiled::tmx {
  ...
  // -------------------------------------------------------------------------------------------
  void json_reader::iterate_data_array (const std::function<void (uint32_t)>& f, const char* name) const {
  ...
  // -------------------------------------------------------------------------------------------
  void json_reader::parse_each_element_of (const char* name, const visitor_t& func) const {
    auto itr = m_pimpl->m_node.find (name);
    if (itr != m_pimpl->m_node.end ()) {
      if (itr->is_array ()) {
  ...
  std::vector<math::point2f> json_reader::parse_points (const char* name) const {
    ...
    for (const auto& obj : node) {
      ...
      const auto x = itr->get<float> ();
      itr = obj.find ("y");
      if (itr == obj.end ()) {
        RAISE_EX("y attribute is missing for points object");
      }
      const auto y = itr->get<float> ();
